...
module.exports.update = async (req, res) => {
    ...
    try {
        ...
        if (connection.status === ConnectionStatus.USER_CONNECTION_ACCEPTED) {
            ...
            await gs_client
                .feed("users", connection.user)
                .follow("users", connection.connected_user._id);
            await gs_client
                .feed("users", connection.connected_user._id)
                .follow("users", connection.user);

            /** receive notifications when the connected user creates an activity  */
            await gs_client
                .feed("notifications", connection.user)
                .follow("users", connection.connected_user._id);
