...
module.exports.show = async (req, res) => {
    const result = {};
    try {
        const education = await Models.UserEducation.find({
            user: req.params.user,
            _id: req.params.id,
        });
        res.status(200);
...
module.exports.create = async (req, res) => {
    ...
    Models.UserEducation.create(req.body, async (error, education) => {
        if (!error) {
            try {
                const user = await Models.User.findByIdAndUpdate(education.user, {
                    $addToSet: {
...
module.exports.update = async (req, res) => {
    const result = {};
    try {
        const education = await Models.UserEducation.findOneAndUpdate(
            {
                user: req.params.user,
                _id: req.params.id,
            },
...
module.exports.delete = async (req, res) => {
    ...
    try {
        ...
        const delete_result = await Models.UserEducation.deleteOne({
            _id: req.params.id,
        });
